...
If unique, [open an issue](https://github.com/webhippie/archiver/issues) and answer the questions so we can understand and reproduce the problematic behavior.
...
We believe that in order to have a healthy codebase we need to abide to a certain code style. We use `v fmt` with Vlang for this matter, which are tools that has proved to be useful. So, before submitting your Pull Request, make sure that `v fmt` is passing for you.

Finally, note that `v fmt` is called on the CI system. This means that your Pull Request will not be merged until the changes are approved.
...
If you've found a vulnerability that is serious, please email to <mailto:thomas@webhippie.de>. If it's not a big deal, a pull request will probably be faster.
